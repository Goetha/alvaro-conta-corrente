...
export function PedagioDrawer({ open, onClose, onSave, initial }) {
  ...
  useEffect(() => {
    if (initial) {
      setFormData({
        ...
        codigo: initial.codigo || '69',
        conta: initial.conta || 'PEDÁGIO',
        valor: initial.valor || ''
      });
    } else {
      setFormData({
    ...
    <>
      ...
      <div className="fixed right-0 top-0 h-full w-full max-w-md bg-white shadow-2xl z-50 flex flex-col">
        <div className="flex items-center justify-between p-4 border-b border-slate-200">
          <h2 className="text-lg font-bold text-slate-800">{initial ? 'Editar Pedágio' : 'Novo Pedágio'}</h2>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Parceiro</label>
