...
const TABLE = 'contas_pagar_manutencao';

export function useManutencao(filters = {}) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      while (true) {
        ...
        if (error) {
          if (error.code === 'PGRST116' || error.message.includes('relation "contas_pagar_manutencao" does not exist')) {
            return [];
          }
          throw error;
        }
