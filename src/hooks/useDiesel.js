...
const TABLE = 'contas_pagar_diesel';

export function useDiesel(filters = {}) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      while (true) {
        ...
        if (filters.search) {
          query = query.or(
            `documento.ilike.%${filters.search}%,fornecedor.ilike.%${filters.search}%,placa.ilike.%${filters.search}%`
          );
        }
