...
async function run() {
  ...
  for (let i = 2; i < lines.length; i++) {
    ...
    // Override the code and account to be "69 - PEDÁGIO" so it shows up nicely if filtered by code.
    // Or we keep original? The user wants Pedágio data. I will keep original but insert to pedagio.
    ...
    records.push({
      ...
      km,
      quantidade,
      codigo: '69', // Force 69 since it's the Pedagio tab
      conta: 'PEDÁGIO',
      valor
    });
  }

  console.log(`Parsed ${records.length} records. Uploading to 'contas_pagar_pedagio'...`);

  await supabase.from('contas_pagar_pedagio').delete().neq('id', '00000000-0000-0000-0000-000000000000');
  ...
  for (let i = 0; i < records.length; i += 200) {
    ...
    const { error } = await supabase.from('contas_pagar_pedagio').insert(batch);
    if (error) {
  ...
  console.log('Import to PEDAGIO completed successfully!');
}
